...
impl<'a> StringLexer<'a> {
    ...
    pub fn lex_escape_complex_parameterised(
        ...
        match escape_name {
            ...
            "code" => {
                ...
                let startpos: Position = self.lexer.pos;
    ...
    pub fn lex_string_escape_simple_parameterised(
        ...
        match escape_char {
            ...
            'o' => {
                ...
                Ok((
                    StringPart::Escape(StringEscape::Byte(
                        u8::from_ascii_radix(&digits, 8).unwrap_or_default() // idk null byte by default
                    )),
                    PositionRange::new(
                        self.lexer.reader.path.into(),
                        begin_state.pos,
            ...
            'u' => {
                ...
                for i in 0..4 {
                    ...
                    if next == '}' {
                        // Shift each digit to the left by however many digits
                        // remain unset (so they dont affect the value)
                        digits.rotate_right(4 - n);
                    }
                ...
                let _closing_brace = self.lexer.expect_char('}')
                    .inspect_err(|_| self.lexer.reset_to_state(begin_state))?;

                let c = char::from_u32(u32::from_ascii_radix(&digits, 16).unwrap()).unwrap();
    ...
    fn lex_string_escape(&mut self, begin_state: LexerState<'a>) -> Result<(StringPart, PositionRange), LexError> {
        ...
        if ONE_CHAR_ESCAPE_SEQUENCES.binary_search(&start).is_err() {
            loop {
    ...
    pub fn lex_raw_string(
        ...
        self.lexer.cur_context = Some(TokenContext::StringChars);

        let mut contents = String::new();

        let content_start_state = self.lexer.state();
        ...
        loop {
            ...
            if next == '`' {
                ...
                // insert one backtick
                if let Some('`') = self.lexer.reader.peek_next_char() {
                    contents.push('`');
        ...
        Ok((
            ...
            ).into()).inspect(|_| self.lexer.cur_context = begin_state.cur_context)
    }

    /// Lex a raw string or normal string.
    pub fn lex_string(&mut self) -> Result<LexOutput, LexError> {
        self.lexer.cur_context = Some(TokenContext::StringChars);
        ...
        let mut content_start_state = self.lexer.state();
        let mut content_end_state = content_start_state;
            ...
            match next {
                '"' => {
                    if acc.len() > 0 {
                        ...
                        parts.push((part, PositionRange::new(
                            self.lexer.reader.path.into(),
                            content_start_state.pos,
                            content_end_state.pos,
                            self.lexer.cur_context
                ...
                '\\' => {
                    ...
                    let escape_or_template = self.lex_string_escape(content_end_state)
                        .inspect_err(|_| self.lexer.reset_to_state(begin_state))?;

                    parts.push(escape_or_template);

                    self.lexer.cur_context = Some(TokenContext::StringChars);
                    content_start_state = self.lexer.state();
